...
io.on('connection', (socket) => {
    ...
    socket.on('set-username', async (username) => {
        ...
        // Thực hiện kết nối an toàn với Khối Bắt Lỗi (try-catch) tránh sập backend
        try {
            await tiktok.connect();
            socket.emit('status', `Đã kết nối: \${username}`);
        } catch (err) {
            console.error("Lỗi kết nối TikTok Live:", err.message);
            socket.emit('status', `Lỗi kết nối: Không tìm thấy Live hoặc thư viện bị chặn!`);
            return; // Dừng tiến trình đăng ký event nếu connect fail
        }
